...
def has_condorcet_winner(profile, alternatives):
    '''
    Determines whether a given profile has a condorcet winner.
    '''
    winners = list()
...
def runSim(num_sims, num_alternatives, num_voters):
    ...
    num_condorcet_winners_percentages = {k: (str(100*v/num_sims)+"%") for k, v in num_condorcet_winners.items()}
...
if __name__ == "__main__":
    ...
    data = []
    columns = ["num_voters", "num_alternatives"] + [str(i) + "_winners" for i in range(num_alternatives_range[1]+1)] + ["has_winner"]
    data.append(columns)
